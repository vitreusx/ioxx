...
YAML::Node merge_yaml(YAML::Node const &defaults, YAML::Node const &overrides) {
  if (!overrides.IsMap()) {
    // If overrideNode is not a map, merge result is overrideNode, unless
    // overrideNode is null
  ...
  // Create a new map 'newNode' with the same mappings as defaultNode, merged
  // with overrideNode
